...
const downloadParams = Type.Object({
  ...
  timeout_ms: Type.Optional(
    Type.Number({
      description:
        '前台阻塞模式整体超时毫秒数，默认 600000（10 分钟）。大文件或慢速网络建议改用 background=true 后台下载。'
    })
  ),
...
export function createDownloadTool(
  ...
  return {
    ...
    async execute(_toolCallId, p, signal, onUpdate) {
      ...
      const dest = p.path ?? join(resolveAgentSessionWorkdir(sessionId), fallbackName)
      ...
      if (signal?.aborted) {
        ...
        return {
          content: [
            {
              type: 'text',
              text: `[已中止] 已下载 ${formatBytes(task.downloadedBytes)} 至 ${dest}（文件不完整）`
            }
          ],
          details: {
            url: p.url,
      ...
      return {
        content: [
          { type: 'text', text: `下载完成：${dest}（${formatBytes(task.downloadedBytes)}${pct}）` }
        ],
        details: {
          url: p.url,
          path: dest,
