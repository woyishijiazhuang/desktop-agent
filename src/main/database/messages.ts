...
/** 消息 CRUD + 压缩/上下文 + 全文搜索。 */
export function createMessagesApi(db: DatabaseSync, deps: MessageApiDeps): MessageApi {
  const api: MessageApi = {
    ...
    forkSession(sourceSessionId: string, upToMessageId: number): Session {
      return transaction(db, () => {
        const source = deps.getSession(sourceSessionId)
        if (!source) throw new Error('会话不存在')
        const target = api.getMessage(upToMessageId)
    ...
    getSessionContext(sessionId: string): SessionContext {
      ...
      const messages =
        session.compressLastIndex !== null
          ? api.listMessagesBySession(sessionId, { afterId: session.compressLastIndex })
          : api.listMessagesBySession(sessionId)
      return {
        session,
