...
const main = async () => {
  ...
  try {
    ...
    const wrapped = await SandboxManager.wrapWithSandboxArgv('bash --noprofile --norc -s', undefined, undefined, undefined, work);
    ...
    record('A 持久 bash 启动且首条命令即时返回', out1 !== null && out1.includes(work), `首字节延迟 ${firstByteAt ? firstByteAt - spawnT0 : '?'}ms，out=${JSON.stringify(out1)}`);
    record('A 首字节延迟 <= 2s（无异常启动慢）', firstByteAt !== null && firstByteAt - spawnT0 <= 2000, `延迟 ${firstByteAt ? firstByteAt - spawnT0 : '无输出'}ms`);
    ...
    record('B 工作区外（HOME）写被 OS 级拒绝', out5 !== null && /exit=[1-9]/.test(out5) && /Operation not permitted|denied/i.test(out5 + errBuf), `out=${JSON.stringify(out5)} err=${errBuf}`);

    send(`cat ${join(secretDir, 'token.txt')}; echo "exit=$?"`);
    const out6 = await waitChunk();
    record('B denyRead 路径读取被拒', out6 !== null && /exit=[1-9]/.test(out6) && /Operation not permitted|denied/i.test(out6 + errBuf), `out=${JSON.stringify(out6)} err=${errBuf}`);
    ...
    const denied = await SandboxManager.wrapWithSandboxArgv('curl -sS -m 10 https://example.org/', undefined, undefined, undefined, work);
    const n1 = await runWrapped(denied, work);
    record('C-1 非白名单域名被拒（example.org）', n1.code !== 0 || /blocked|denied|not permitted|403/i.test(n1.out + n1.err), `code=${n1.code} out=${n1.out.trim().slice(0, 80)} err=${n1.err.trim().slice(0, 120)}`);
    ...
    record('C-2 allowlist 域名放行（example.com=200）', n2.code === 0 && /200/.test(n2.out), `code=${n2.code} out=${n2.out.trim()} err=${n2.err.trim().slice(0, 120)}`);
