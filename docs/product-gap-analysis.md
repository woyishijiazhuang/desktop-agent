...
10. ◐ 自动更新（electron-updater 已实现检查/下载/安装/启动自动检查，仅打包版）—— 待做：真实 publish 源 + macOS notarize（分发前必做）
11. ✅ 开机自启 / 窗口置顶开关
...
24. ⏳ 单元测试 + CI（含 E2E）—— 待做
25. 🔭 浏览器操作 / Computer Use —— 远期（需引入 playwright/CDP 等重型框架）；代码沙箱已 ◐（bash 进程级隔离落地，见 [bash-sandbox-research.md](bash-sandbox-research.md)）
