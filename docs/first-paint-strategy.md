...
4. 设置窗口「内容就绪后再显示」（2026-09-09，有限等待方案，见 §9.4）——show:false + did-finish-load 后显示，did-fail-load / 1.2s 超时双兜底；无常驻内存。范围仅设置窗口，工作区窗口维持随建随显。

已决策不实施：
- 骨架屏（方案一）：用户明确不要。
- 设置窗口**常驻预载**方案：不采用（常驻 30–50MB 渲染进程内存，用户不接受；与 §9.4 的零常驻方案相比无必要）。
...
- 2026-09-09（三）：实施**设置窗口「内容就绪后再显示」**（有限等待方案，§9.4），用户顾虑（常驻内存 / 等待过久 / 卡住不显示）分别以「零常驻 + 超时与失败双兜底」消除。
- 待办 / 待决策：
  - [ ] （可选）如需工作区首窗同样「内容就绪再显示」，可复用 §9.4 的 reveal 逻辑（当前工作区窗口随建随显）；
...
3. **「窗口与内容一起出现」采用零常驻的有限等待方案（已实施，§9.4）**；另一候选「常驻隐藏设置窗口」因常驻 30–50MB 渲染进程被否。

### 9.4 设置窗口「内容就绪后再显示」（已实施，2026-09-09）

- 动机：设置窗口 `show: true` 随建随显 → 每次打开先见空白窗 ~0.2s（慢时更久），与其他应用「窗口与 UI 一起出现」体验差距大。
- 方案（[window-manager.ts](file:///Users/hupengfei/Documents/my-app/src/main/service/window-manager.ts#L289-L407)）：设置窗口 `show: false` 隐藏创建，内容视图 `did-finish-load`（入口已执行、Vue 已挂载）后 `win.show()` + focus。
- 双兜底消除「等太久 / 卡住永不显示」：
  - `did-fail-load` → 立即显示（让错误页可见）；
  - 超时 `SETTINGS_SHOW_TIMEOUT_MS = 1200ms` → 强制显示（最差退回今天的「先窗后内容」，不劣于现状）。
- 边界：`win.once('closed')` 清理超时 timer；`reveal` 幂等（guard `state.revealed` + `win.isDestroyed()`）。
- 代价与收益：正常点击比之前多等约 0.2s 才看到窗口，但不再看到空白窗；无常驻内存（仅点开设置那一刻建窗口，与现状一致）。
- 实现要点：BaseWindow 无 `ready-to-show`（仅 BrowserWindow 有），故以 contentView 的 `did-finish-load` 作为“内容就绪”信号，未用 paint（首帧可能在纯背景色时即产生，仍会透出空白观感）。
