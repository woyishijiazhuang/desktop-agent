...
#### 4. ~~缺少子 Agent / Task 系统~~ ✅ 已实现（2026-09）

- **已补齐**：`task` 工具委派 `plan`（只读白名单）/ `general`（全量工具）两类子代理，独立上下文运行并回传结果，主代理同轮可并行多个委派。见文末第十批实现记录
- **Claude Code**：`Task` 可启动专门子 Agent（探索型、规划型、通用型），并行执行独立任务
- **已知边界**：子代理为**同步前台**执行（主 Agent `await` 完成后继续），后台化并行执行作为未来方向记录（见文末）
...
#### 9. ~~`edit_file` 缺少模糊匹配能力~~ ✅ 已实现近似候选诊断（2026-08-15）

- **已补齐**：匹配失败时给出近似候选诊断——空白差异 / 大小写差异可唯一定位时，错误信息直接附上文件原文片段与行号，并检测行号前缀误粘贴（见文末第三批实现记录）
- **现状**：不做自动纠错替换，仅提供诊断帮助模型下一轮复制修正
...
| 问题         | 现状                    | 建议                                                                                                                                                                                                                           |
| ------------ | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 读取前置检查 | ⛔ 已刻意移除           | 曾于 2026-08-15 加入「目标文件已存在则拒绝写入并提示先 read_file」，但其语义是**无论如何都不让 write**，存在歧义，已移除并改为**允许不 read 直接 write**；覆盖写丢失旧内容的风险由文件撤销方案兜底（见 `file-undo-design.md`） |
...
第二阶段（效率提升，解决「好不好用」）✅ 4、5 已完成（2026-08-15）:
  ...
  ├── 6. 子 Agent / Task 系统 ✅（task 工具 + plan/general 子代理，见第十批实现记录）
  └── 7. TodoWrite 任务管理
...
- **write_file 读取前置检查**（`write-file.ts`）：~~目标文件已存在时拒绝写入并提示先 read_file~~ —— 该检查的语义是**无论如何都不让 write**，存在歧义，**已刻意移除**，改为允许不 read 直接 write（覆盖写风险由文件撤销方案兜底，见 `file-undo-design.md`）；**保留单次写入 1MB 上限**，超限提示改用 edit_file 分段编辑
